/*
 * extended.js -- Tier 1 extended APIs for qjs -x/--extended mode
 *
 * Loaded by qjs.c when --extended is active.  Installs web/Node-
 * compatible APIs that are not part of stock quickjs-ng:
 *
 *   console.error/warn/info/debug/assert/dir/table
 *   globalThis.process  (argv, env, exit, platform, cwd, chdir, ...)
 *   TextEncoder, TextDecoder  (UTF-8 + ISO-8859-1/Latin-1)
 *   URL, URLSearchParams      (WHATWG, subset)
 *   AbortController, AbortSignal
 *   qjs.path module registration helper
 *
 * Implemented entirely in JS for simplicity; can be optimized to
 * native C classes later.
 */
...
(function() {
    ...
    function write(stream, ...args) {
        /* Use existing print machinery which honours Symbol.for('qjs.inspect') */
        for (let i = 0; i < args.length; i++) {
            ...
            if (typeof a === 'string') {
                ...
                /* Fall back to std.__printObject if available */
    ...
    globalThis.console.error = (...args) => write(std.err, ...args);
    globalThis.console.warn  = (...args) => write(std.err, ...args);
    globalThis.console.info  = (...args) => write(std.out, ...args);
    globalThis.console.debug = (...args) => write(std.out, ...args);
    ...
    globalThis.console.table = (data) => {
        ...
        const rows = data.map((r, i) => {
            const row = [String(i)];
            for (const c of cols) {
                row.push(r && typeof r === 'object' ? String(r[c] ?? '') : String(r));
            }
            return row;
        });
    ...
    /* console.group / groupEnd / time / timeEnd - basic no-op stubs */
    let _groupDepth = 0;
    globalThis.console.group = (...args) => {
        if (args.length) write(std.out, ...args);
        _groupDepth++;
    };
    globalThis.console.groupEnd = () => { if (_groupDepth > 0) _groupDepth--; };
...
globalThis.process = {
    ...
    pid: 1,   /* AmigaOS doesn't expose pids meaningfully */
    ppid: 0,
    ...
    hrtime(prev) {
        const now = os.now();        /* microseconds since epoch */
...
/* ==========================================================
 * queueMicrotask (if not already provided)
 * ========================================================== */
    ...
    globalThis.queueMicrotask = (cb) => {
        Promise.resolve().then(() => {
            try { cb(); } catch (e) {
                ...
                try { console.error('queueMicrotask error:', e); } catch (_) {}
            }
        });
    };
}
...
/* ==========================================================
 * TextEncoder / TextDecoder  (UTF-8 + Latin-1)
 * ========================================================== */
...
class TextDecoder {
    ...
    decode(input) {
        ...
        if (input instanceof ArrayBuffer) {
            ...
            throw new TypeError('decode() requires a BufferSource');
        }
        ...
        while (i < bytes.length) {
            ...
            if (b < 0x80) {
                ...
                if (i >= bytes.length) { if (this._fatal) throw new TypeError('truncated UTF-8'); cp = 0xFFFD; break; }
                cp = ((b & 0x1F) << 6) | (bytes[i++] & 0x3F);
            } else if ((b & 0xF0) === 0xE0) {
                if (i + 1 >= bytes.length) { if (this._fatal) throw new TypeError('truncated UTF-8'); cp = 0xFFFD; break; }
                cp = ((b & 0x0F) << 12) | ((bytes[i++] & 0x3F) << 6) | (bytes[i++] & 0x3F);
            } else if ((b & 0xF8) === 0xF0) {
                if (i + 2 >= bytes.length) { if (this._fatal) throw new TypeError('truncated UTF-8'); cp = 0xFFFD; break; }
...
/* ==========================================================
 * URL and URLSearchParams  (WHATWG, subset)
 * ========================================================== */
...
class URLSearchParams {
    constructor(init) {
        ...
        if (typeof init === 'string') {
            ...
            for (const pair of s.split('&')) {
                ...
                name = _pctDecode(name.replace(/\+/g, ' '));
                value = _pctDecode(value.replace(/\+/g, ' '));
    ...
    _notify() {
        if (this._url) this._url._syncSearchFromParams();
    }
}
...
class URL {
    ...
    static _parse(str, base) {
        str = str.trim();
        /* Very simplified parser -- sufficient for http(s)://, file://,
           and common absolute URLs.  Not a full WHATWG implementation. */
        const o = {
        ...
        if (m) {
            ...
            /* handle relative path merging */
            if (s.startsWith('//')) {
                s = o.scheme + ':' + s;
                return URL._parse(s, null);
            } else if (s.startsWith('/')) {
                o.path = s;
                o.query = ''; o.fragment = '';
                s = '';
            } else if (s.startsWith('?')) {
                o.query = s.substring(1);
                o.fragment = '';
                s = '';
            } else if (s.startsWith('#')) {
                o.fragment = s.substring(1);
                s = '';
            } else if (s.length > 0) {
                /* relative path */
        ...
        if (s.startsWith('//')) {
            ...
            const closeBr = hostStr.lastIndexOf(']');  /* IPv6 bracket */
        ...
        if (hashIdx >= 0) {
            o.fragment = s.substring(hashIdx + 1);
            s = s.substring(0, hashIdx);
        }
        const queryIdx = s.indexOf('?');
        if (queryIdx >= 0) {
            o.query = s.substring(queryIdx + 1);
    ...
    set href(v) {
        ...
        Object.assign(this, {
            _scheme: parsed.scheme, _username: parsed.username, _password: parsed.password,
            _host: parsed.host, _port: parsed.port, _path: parsed.path,
...
/* ==========================================================
 * structuredClone (shallow wrapper using bjson round-trip)
 * ========================================================== */

try {
    /* Lazy import to avoid hard dependency if bjson not loaded */
    const bjson = await import('qjs:bjson');
    globalThis.structuredClone = (value) => bjson.read(bjson.write(value));
} catch (e) {
    /* Fallback: JSON-based deep clone (loses Dates/Maps/etc) */
    globalThis.structuredClone = (value) => JSON.parse(JSON.stringify(value));
}


/* ==========================================================
 * Mark extended mode active
 * ========================================================== */

globalThis.qjs = globalThis.qjs || {};
globalThis.qjs.extended = true;
globalThis.qjs.version = '0.65';


/* ==========================================================
 * path module (AmigaOS-aware)
 *
 * Exposed as globalThis.path (and via 'qjs:path' if a future
 * version registers it as a module).  Handles both POSIX '/'
 * and AmigaOS 'Volume:' / 'Volume:dir/file' paths.
 * ========================================================== */

const path = (function() {
    function isAbsolute(p) {
        if (typeof p !== 'string' || p.length === 0) return false;
        /* AmigaOS absolute: Volume: or Volume:path or :path */
    ...
    /* Split a path into volume prefix, directory components, and
     * filename.  "RAM:foo/bar.txt" -> {volume:"RAM:", parts:["foo"], file:"bar.txt"} */
    function _split(p) {
        let volume = '';
        let rest = p;
        const colon = rest.indexOf(':');
        if (colon >= 0) {
            volume = rest.substring(0, colon + 1);
    ...
    function extname(p) {
        ...
        if (dot <= 0) return '';  /* no ext, or leading-dot file like .foo */
    ...
    function join(...segments) {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            if (split.volume) {
                /* New absolute path resets everything */
                volume = split.volume;
    ...
    function resolve(...segments) {
        ...
        if (!resolvedAbs) {
            try {
                const cwd = (globalThis.os && globalThis.os.getcwd) ? globalThis.os.getcwd() : '';
    ...
    function relative(from, to) {
        ...
        to = resolve(to);
        if (from === to) return '';
        const fs = _split(from);
        const ts = _split(to);
        if (fs.volume !== ts.volume) return to;  /* can't make relative across volumes */
