...
if [ -n "${BASH_SOURCE[0]+x}" ] && [ -n "${BASH_SOURCE[0]}" ]; then
    _AMIGA_BIN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
elif [ -n "${ZSH_VERSION}" ]; then
    _AMIGA_BIN_DIR="${0:A:h}"
else
    _AMIGA_BIN_DIR="$(pwd)/bin"
fi
_AMIGA_PROJECT_ROOT="$(dirname "$_AMIGA_BIN_DIR")"
_AMIGA_QJS_ROOT="$_AMIGA_PROJECT_ROOT/quickjs-master"
_AMIGA_INCLUDE="$_AMIGA_PROJECT_ROOT/include"
_AMIGA_SRC="$_AMIGA_PROJECT_ROOT/src"
_AMIGA_VAMOS_CFG="$_AMIGA_BIN_DIR/vamos_build.cfg"
...
# amiga_compile_src FILE [EXTRA_FLAGS...]
# Compile a file from src/ directory (FPU build).
amiga_compile_src() {
    _amiga_check_env || return 1
    local file="$1"; shift
    amiga_clear
...
# amiga_compile_src_soft FILE [EXTRA_FLAGS...]
amiga_compile_src_soft() {
    _amiga_check_env || return 1
    local file="$1"; shift
    amiga_clear
    vamos \
...
# amiga_compile_src_cpu FILE CPU [EXTRA_FLAGS...]
amiga_compile_src_cpu() {
    _amiga_check_env || return 1
...
amiga_link() {
    ...
    vamos \
        ...
        -V qjs:"$_AMIGA_QJS_ROOT" \
        -V src:"$_AMIGA_SRC" \
        -V out:"$_AMIGA_PROJECT_ROOT/amiga" \
        sc:c/slink \
        sc:lib/c.o \
...
amiga_build_040() {
    ...
    amiga_compile_src_cpu amiga_compat.c 68040 &&
    ...
    amiga_compile_src_cpu amiga_ssl.c 68040 MEMSIZE=HUGE IDLEN=80 &&
...
amiga_build_060() {
    ...
    amiga_compile_src_cpu amiga_compat.c 68060 &&
    ...
    amiga_compile_src_cpu amiga_ssl.c 68060 MEMSIZE=HUGE IDLEN=80 &&
