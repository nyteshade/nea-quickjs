...
#include "screensave_rev.h"
UBYTE vers[] = VERSTAG;
UBYTE Copyright[] = VERS " - Saves screen as ILBM - Freely Redistributable";

char *usage =
 "Usage: screensave filename (filename -c[unit] for clipboard)\n" 
 "Opts: QUIET NODELAY NOICON NOMONITOR VIEWPORT SEQUENCE (adds number to name)\n"
 "Saves front screen (or ViewPort) after 10-sec delay (unless NODELAY).\n";
