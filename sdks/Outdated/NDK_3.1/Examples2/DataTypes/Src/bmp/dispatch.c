...
BOOL ASM GetBMP (REG (a6) struct ClassBase * cb, REG (a0) Class * cl, REG (a2) Object * o, REG (a1) struct TagItem * attrs)
{
	...
	if (Read (fh, &bf, BFSIZE) == BFSIZE)
	{
		...
		if (Read (fh, &bi, BISIZE) == BISIZE)
		{
		    ...
		    aspectX = SWAPL (bi.biXPelsPerMeter);
		    aspectY = SWAPL (bi.biYPelsPerMeter);
		    ...
		    if(((aspectX + aspectY - 1) / aspectY) == 2)
			modeid = HIRES_KEY;

		    if(((aspectY + aspectX - 1) / aspectX) == 2)
			...
			if (Read (fh, &rgb, QSIZE) == QSIZE)
			{
			    ...
			    cregs[n * 3 + 0] = rgb.rgbRed   * 0x01010101;
			    cregs[n * 3 + 1] = rgb.rgbGreen * 0x01010101;
			    cregs[n * 3 + 2] = rgb.rgbBlue  * 0x01010101;
			}
			else
			    return (FALSE);
		    }
