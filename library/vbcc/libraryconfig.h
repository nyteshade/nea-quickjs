...
#define LIBRARY_VERSION_STRING \
    "\0$VER: quickjs." QJS_STR(QJS_VARIANT_NAME) ".library 0.140 (21.4.2026)\r\n"
#define LIBRARY_VERSION_OUTPUT &LIBRARY_VERSION_STRING[7]
#define LIBRARY_VERSION   140  /* packed: major=0, revision=140 (Phase B bugfixes caught by user testing react_hello on Amiga — "does nothing visible". Three real bugs: (1) Window.ATTRS used nonexistent WINDOW_CloseGadget/SizeGadget/DragBar/DepthGadget tag IDs in the 0x85025019..0x8502501C range — real tags are WA_CloseGadget/SizeGadget/DragBar/DepthGadget at WA_Dummy+0x21..+0x1E. With wrong tags the window had no decorations and probably refused to open. (2) Layout.addChild was dispatching OM_ADDMEMBER post-construction; Reaction's layout.gadget wants LAYOUT_AddChild as a TAG at NewObject construction time (it may repeat in the same tag list, one per child). Without proper registration the layout has zero minimum size → invisible window. (3) BOOPSIBase lacked an _extraPairs hook for repeated-tag injection needed by Layout's children-as-tags pattern. Fix: WA_* tags, Layout constructor builds LAYOUT_AddChild pairs via _extraPairs, BOOPSIBase._buildTagList honors them.) */
